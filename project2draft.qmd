...
Prior to model fitting, a correlation matrix is computed to assess potential relationships between numerical explanatory variables.
...
@fig-boxplot5 shows a boxplot of mean altitude(in meters) of coffee batch being classified as Good and Poor quality.
...
The model suggests that Aroma, Flavor, and Acidity are the most important factors in determining coffee quality.
...
The model remains strong and statistically valid, with Aroma, Flavor, and Acidity being the strongest predictors of coffee quality.
...
Need to perform further model comparasion using deviance.
...
However, many country of origin variables (e.g. Burundi, China, Ethiopia, Vietnam) are not significant, meaning they do not strongly predict coffee quality.
Altitude, Harvest Year, and Category Two Defects remain non-significant, suggesting they should be removed for the further analysis.

Following the incorporation of all variables into the model, an enhancement in R square, that is to say an improvement in interpretability, was observed, concomitant with a diminution in AIC.
...
The intercept is 120, menaing that when all explanatory variables are zero, the log-odds of being classified as poor quality is extremely high.
...
Similarly, the log-odds of being classified as poor quality for the batch will decrease by 7.02 and 4.42 for one unit incrase in flavor and acidity grades respectively when keep all other variables constant.

Mean altitude has a very small negative coefficient compared to the previous three variables, so the log-odds of a coffee batch being Poor slightly decrease by 0.0008 as one unit inrease in mean altitude when hold other variables kept unchanged.
...
For @tbl-logoddsCI, since all the intervals($[-8.8354,-5.4369]$ for flavor grade, $[-5.9336,-3.1506]$ for aroma grade, $[-5.5577,-2.8939]$ for acidity grade, $[-0.0013,-0.0003]$ for mean altitude) don't contain zero thus indicates that the all explanatory variables in Model 3 are significant.
...
Notice that flavor has the widest range of negative values whereas mean altitude has the narrowst confidence range.
...
Finally, the odds for mean altitude(meters) is $9.99\times10^{-1}$, nearly approach to 1, indicating that for every 1 unit increase in mean altitude(meters), it decreases the odds of being classified as poor quality by 0.08% when hold all other vaiables unchanged.
...
From @tbl-oddsCI, most of the intervals($[0.00015, 0.0044]$ for flavor grade, $[0.0026,0.0428]$ for aroma grade, $[0.0039, 0.0554]$ for acidity grade, $[0.9987, 0.9997]$ for mean altitude) don't contain one thus indicate that the explanatory variables in model 3 are significant.
...
Then, Altitude has a moderate but still significant effect on the quality score of coffee batch.

Therefore, focus on improving flavor, aroma and acidity grades, growing coffee at low altitudes, the local coffee farmers will have greater chance that their coffee being classified as high quality (Good).
